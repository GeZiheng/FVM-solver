...
struct ConvDiffSetup {
  ...
  ConvDiffSetup(Index n, Scalar gammaVal)
    : mesh(n, n, 0.0, 0.0, 1.0, 1.0), gamma(mesh, "gamma"), velocity(mesh, "u")
  {
...
TEST_CASE("Convection: central differencing is second-order accurate (1D conv-diff, Pe = 10)")
{
  const Scalar gammaVal = 0.1;
  Scalar prevErr = -1.0, prevH = 0.0;
...
TEST_CASE("Convection: upwind stays bounded where central oscillates (high cell Pe)")
{
  ...
  for (auto scheme : { ConvectionScheme::Upwind, ConvectionScheme::Central }) {
    ...
    if (scheme == ConvectionScheme::Upwind) {
      ...
      // Sanity check that the test case is actually in the oscillatory regime.
      CHECK((lo < -1e-6 || hi > 1.0 + 1e-6));
    }
  }
}

TEST_CASE("Convection: constant field is preserved (consistency with uniform flow)")
{
