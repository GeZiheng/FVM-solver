...
TEST_CASE("Diffusion: matrix is SPD with Dirichlet BC (symmetry + CG convergence)")
{
...
TEST_CASE("Diffusion: manufactured solution converges at second order")
{
  ...
  for (Index n : { 10, 20, 40 }) {
    ...
    BoundaryField bc; // north/south default zero-flux (consistent: dphi/dy = 0)
    bc.set(BoundaryField::West, BCType::Dirichlet, 0.0);
...
TEST_CASE("Diffusion: fully closed domain (all zero Neumann) conserves the mean")
{
