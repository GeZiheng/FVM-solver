  ...
  for (Index P = 0; P < nCells; ++P) {
    for (int face = 0; face < 4; ++face) {
      ...
      if (N != nCells) {
        ...
        const Scalar F = interiorFaceFlux(mesh, velocity, rho, P, N, face);

        if (scheme == ConvectionScheme::Upwind) {
          if (F > 0.0) { // flow P -> N: face value = phi_P
            A.insert(P, P, F);
            A.insert(N, P, -F);
          } else { // flow N -> P: face value = phi_N
            A.insert(N, N, -F);
            A.insert(P, N, F);
          }
        } else { // Central: face value = 0.5 * (phi_P + phi_N)
        ...
        // Boundary face.
        const Scalar Fb = boundaryFaceFlux(mesh, velocity, rho, P, face);
