...
SparseMatrix::SparseMatrix(Index rows, Index cols) : mat_(rows, cols) {}
...
void SparseMatrix::insert(Index row, Index col, Scalar value)
{
  ...
  triplets_.emplace_back(static_cast<Eigen::Index>(row), static_cast<Eigen::Index>(col), value);
}
...
{
  if (!finalized_) {
    throw std::runtime_error("SparseMatrix must be finalized before accessing native matrix");
  }
  return mat_;
}
