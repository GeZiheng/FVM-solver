...
class CartesianMesh {
public:
  CartesianMesh(Index nx, Index ny, Scalar xMin, Scalar yMin, Scalar xMax, Scalar yMax);

  // Grid dimensions
  Index nx() const { return nx_; }
  Index ny() const { return ny_; }
  Index cellCount() const { return nx_ * ny_; }
  ...
  // Index mapping
  Index cellIndex(Index i, Index j) const { return j * nx_ + i; }
  ...
  // Domain bounds
  Scalar xMin() const { return xMin_; }
  Scalar yMin() const { return yMin_; }
  Scalar xMax() const { return xMax_; }
  Scalar yMax() const { return yMax_; }
