...
public:
  ScalarField(const CartesianMesh& mesh, const std::string& name = "")
    : mesh_(mesh), name_(name), data_(mesh.cellCount())
  {
    data_.setZero();
  }
  ...
  Scalar& operator()(Index cell) { return data_[cell]; }
  const Scalar& operator()(Index cell) const { return data_[cell]; }

  // Access by (i, j) coordinates
  Scalar& operator()(Index i, Index j) { return data_[mesh_.cellIndex(i, j)]; }
  const Scalar& operator()(Index i, Index j) const { return data_[mesh_.cellIndex(i, j)]; }
  ...
  void setConstant(Scalar value) { data_.setConstant(value); }
...
public:
  VectorField(const CartesianMesh& mesh, const std::string& name = "")
    : mesh_(mesh), name_(name), u_(mesh, name + "_u"), v_(mesh, name + "_v") {}
